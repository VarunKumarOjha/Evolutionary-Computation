...
public class JavaRand extends java.util.Random {
  ...
  // generate Gamma(1,1)
    ...
    public int rand(int low, int high) {
        return (int) (Math.floor(rand() * (high - low) + low + .5));
    }

    public int rand(int range) {
        return (int) (Math.floor(rand() * range));
    }

    /**
     * Generating permutation within a range of numbers random vector between
     * min and Ma
     *
